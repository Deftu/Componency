...
public abstract class Component<T : Component<T, C>, C : ComponentProperties<T, C>>(
...
) : ComponentPlatformAware<T, C>, ComponentRoot<T>, ComponentEventConsumer<T> {
    ...
    // Pointer state
    private val pointerTracker = PointerTracker<T, C>(this)
    ...
    public fun <T : Trait> getTrait(type: KClass<T>): T? {
        return traits.firstOrNull { it::class == type } as? T
    }
    ...
    public override fun onPointerClick(listener: PointerClickEvent.() -> Unit): T = apply {
        eventBus.pointerClickListeners.add(listener)
    } as T

    public override fun onPointerRelease(listener: PointerReleaseEvent.() -> Unit): T = apply {
        eventBus.pointerReleaseListeners.add(listener)
    } as T

    public override fun onPointerDrag(listener: PointerDragEvent.() -> Unit): T = apply {
        eventBus.pointerDragListeners.add(listener)
    } as T

    public override fun onScroll(listener: ScrollEvent.() -> Unit): T = apply {
        eventBus.scrollListeners.add(listener)
    } as T

    public override fun onHover(listener: HoverEvent.() -> Unit): T = apply {
        eventBus.hoverListeners.add(listener)
    } as T

    public override fun onUnhover(listener: HoverEvent.() -> Unit): T = apply {
        eventBus.unhoverListeners.add(listener)
    } as T

    public override fun onKeyPress(listener: KeyPressEvent.() -> Unit): T = apply {
        eventBus.keyPressListeners.add(listener)
    } as T

    public override fun onKeyRelease(listener: KeyReleaseEvent.() -> Unit): T = apply {
        eventBus.keyReleaseListeners.add(listener)
    } as T

    public override fun onCharType(listener: CharTypeEvent.() -> Unit): T = apply {
        eventBus.charTypeListeners.add(listener)
    } as T

    override fun onFocus(listener: () -> Unit): T = apply {
        eventBus.focusListeners.add(listener)
    } as T

    override fun onUnfocus(listener: () -> Unit): T = apply {
        eventBus.unfocusListeners.add(listener)
    } as T

    override fun removePointerClickListener(listener: PointerClickEvent.() -> Unit): T = apply {
        eventBus.pointerClickListeners.remove(listener)
    } as T

    override fun removePointerReleaseListener(listener: PointerReleaseEvent.() -> Unit): T = apply {
        eventBus.pointerReleaseListeners.remove(listener)
    } as T

    override fun removePointerDragListener(listener: PointerDragEvent.() -> Unit): T = apply {
        eventBus.pointerDragListeners.remove(listener)
    } as T

    override fun removeScrollListener(listener: ScrollEvent.() -> Unit): T = apply {
        eventBus.scrollListeners.remove(listener)
    } as T

    override fun removeHoverListener(listener: HoverEvent.() -> Unit): T = apply {
        eventBus.hoverListeners.remove(listener)
    } as T

    override fun removeUnhoverListener(listener: HoverEvent.() -> Unit): T = apply {
        eventBus.unhoverListeners.remove(listener)
    } as T

    override fun removeKeyPressListener(listener: KeyPressEvent.() -> Unit): T = apply {
        eventBus.keyPressListeners.remove(listener)
    } as T

    override fun removeKeyReleaseListener(listener: KeyReleaseEvent.() -> Unit): T = apply {
        eventBus.keyReleaseListeners.remove(listener)
    } as T

    override fun removeCharTypeListener(listener: CharTypeEvent.() -> Unit): T = apply {
        eventBus.charTypeListeners.remove(listener)
    } as T

    override fun removeFocusListener(listener: () -> Unit): T = apply {
        eventBus.focusListeners.remove(listener)
    } as T

    override fun removeUnfocusListener(listener: () -> Unit): T = apply {
    ...
    public open fun handlePointerClick(x: Double, y: Double, button: MouseButton) {
        ...
        debugger?.handlePointerClick(button)
    ...
    public open fun handlePointerDrag(x: Double, y: Double) {
        if (
        ...
        children.forEach { child -> child.handlePointerDrag(x, y) }
    }

    public open fun handleScroll(amount: Double) {
        if (amount == 0.0) {
            return
        }
